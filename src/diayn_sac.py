...
def run_experiment(args):
    ...
    sw = SummaryWriter(log_dir=conf.log_dir + f"/sac_{args.env}_{timestamp}", comment=f"PPO Discriminator, env_name:{args.env}")
    ...
    env = DummyVecEnv([lambda: RewardWrapper(SkillWrapper(Monitor(gym.make(args.env), conf.log_dir + f"/sac_{args.env}_{timestamp}"), conf.n_z, max_steps=conf.max_steps), d, conf.n_z)])
    ...
    eval_env = RewardWrapper(SkillWrapper(Monitor(gym.make(args.env), conf.log_dir + f"/sac_{args.env}_{timestamp}" + f"/eval_results"), conf.n_z), d, conf.n_z)
